...
export const ManageRestoProvider: React.FC<ManageRestoProviderProps> = ({
  children,
}) => {
  ...
  const addItemToCart = (meal: Meal) => {
    setCartItems((prevCart) => {
      const existingItem = prevCart.find(
        (item) =>
          item.mealId === meal.mealId && item.restaurantId === meal.restaurantId
      );
  ...
  useEffect(() => {
    ...
    const fetchedOrders = async () => {
      try {
        const response = await fetch(
          `https://citypulse.runasp.net/api/User/AllOrdersByUserID?UserId=${
    ...
    const fetchedAppointments = async () => {
      try {
        const response = await fetch(
          `https://citypulse.runasp.net/api/User/AllAppointmentsByUserID?UserId=${
            store.getState().auth.user?.id
          }`,
          {
            method: "GET",
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `https://citypulse.runasp.net/api/Restaurant/AllMeals/${id}`
        );
        const data = await response.json();
        if (response.ok) {
